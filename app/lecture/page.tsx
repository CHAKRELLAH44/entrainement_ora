...
export default function LecturePage() {
  ...
  useEffect(() => {
    ...
    getAllReadingProgress(currentUser).then(async (progresses) => {
      ...
      for (const book of BOOKS) {
        reviewed[book.id] = await hasCompletedReview(currentUser, book.id);
      }
  ...
  if (loading) return (
    <div className="page-wrapper">
      <div className="card" style={{ textAlign: "center" }}><p>⏳ Chargement...</p></div>
    </div>
  );
      ...
      <div className="card">
        <div className="nav-top">
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.8rem", color: "var(--btn)", fontFamily: "Lato, sans-serif", fontWeight: "700" }}
          >
        ...
        {!showHistory && (
          <>
            ...
            <div style={{ background: "var(--bg)", borderRadius: "12px", padding: "0.75rem 1rem", marginBottom: "1.25rem", borderLeft: "3px solid #F98F0B" }}>
              <p style={{ fontSize: "0.8rem", color: "var(--muted)", margin: 0, lineHeight: "1.5" }}>
📌 Choisis un livre à découvrir. Après l'avoir terminé, tu dois écrire un bilan d'au moins 100 mots + ajouter 2 mots que tu viens de découvrir avec leur explication. Ensuite, tu pourras choisir un autre livre.              </p>
            </div>
                        ...
                        {status === "reviewed" && (
                          <div style={{ marginTop: "0.75rem", background: "#27AE6022", borderRadius: "8px", padding: "0.5rem 0.75rem" }}>
                            <p style={{ fontSize: "0.8rem", color: "#27AE60", margin: 0, fontWeight: "700" }}>
                              ✅ Lu et evalue le {new Date(progressMap[book.id]?.completedAt || "").toLocaleDateString("fr-FR")}
                            </p>
                          </div>
                        )}
...
function ReviewCard({ review }: { review: ReadingReview }) {
  ...
  return (
    <div style={{ background: "var(--bg)", borderRadius: "16px", padding: "1rem", marginBottom: "0.75rem", border: "1.5px solid var(--border)" }}>
      ...
      {review.summaryCorrected && (
        <div style={{ marginBottom: "0.75rem" }}>
          {!showCorrection ? (
            <button onClick={() => setShowCorrection(true)} style={{ width: "100%", padding: "0.5rem", borderRadius: "8px", border: "1.5px solid var(--btn)", background: "transparent", color: "var(--btn)", fontFamily: "Lato, sans-serif", fontSize: "0.82rem", fontWeight: "700", cursor: "pointer" }}>
              ✅ Voir la correction
            </button>
          ) : (
      ...
      {review.wordsLearned && review.wordsLearned.length > 0 && (
        <div>
          {!showWords ? (
            <button onClick={() => setShowWords(true)} style={{ width: "100%", padding: "0.5rem", borderRadius: "8px", border: "1.5px solid #27AE60", background: "transparent", color: "#27AE60", fontFamily: "Lato, sans-serif", fontSize: "0.82rem", fontWeight: "700", cursor: "pointer" }}>
